...
def register(app: typer.Typer) -> None:
    ...
    app.command("proxy-add")(proxy_add)
...
def proxy_add(
    ...
    create_cert: bool = typer.Option(
        False,
        "--create-cert",
        help="Create a new Cloudflare certificate before creating the proxy host",
    ),
    cloudflare_api_token: Optional[str] = typer.Option(
        None,
        help="Cloudflare API token (used with --create-cert)",
        envvar="CLOUDFLARE_API_TOKEN",
    ),
    certificate_name: Optional[str] = typer.Option(
        None,
        help="Human-friendly certificate name (used with --create-cert)",
    ),
    propagation_seconds: int = typer.Option(
        30,
        help="DNS propagation wait in seconds (used with --create-cert)",
    ),
    ...
    """Create a proxy host, optionally creating a Cloudflare certificate first."""
    try:
        _validate_proxy_add_inputs(create_cert=create_cert, certificate_id=certificate_id)
...
def _validate_proxy_add_inputs(*, create_cert: bool, certificate_id: Optional[int]) -> None:
    if create_cert and certificate_id is not None:
        raise NPMError("Choose either --certificate-id or --create-cert, not both.")
